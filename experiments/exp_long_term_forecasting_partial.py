    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                # subseq_len = 3
                # num_subsequences = batch_x.size(1) // subseq_len
                # subseq_losses = []
                batch_x = batch_x.float().to(self.device)
                ...
                # decoder input
                dec_inp = torch.zeros_like(batch_y[:, -self.args.pred_len:, :]).float()#torch.Size([32, 24, 6])
                dec_inp = torch.cat([batch_y[:, :self.args.label_len, :], dec_inp], dim=1).float().to(self.device)#torch.Size([32, 24, 6])
